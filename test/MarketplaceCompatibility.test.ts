...
describe("Marketplace Compatibility (ERC721 & ERC1155)", async function () {
  ...
  async function deployAll() {
    ...
    return { marketplace, nexus, mock1155, nexusToken, owner, seller, buyer };
  }

  it("Should handle ERC721 offers and acceptance", async function () {
    const { marketplace, nexus, nexusToken, seller, buyer, owner } = await deployAll();

    // Fund buyer with NexusToken
    await nexusToken.write.transfer([buyer.account.address, parseEther("10")], { account: owner.account });

    // Seller mints an NFT
    await nexus.write.donate(["sellable nft", ""], { account: seller.account, value: parseEther("0.1") });
    const tokenId = 0n;

    // Buyer makes an offer
    const offerPrice = parseEther("1");
    await nexusToken.write.approve([marketplace.address, offerPrice], { account: buyer.account });
    await marketplace.write.createOffer([nexus.address, tokenId, 1n, offerPrice], { account: buyer.account });

    // Seller approves and accepts
    await nexus.write.approve([marketplace.address, tokenId], { account: seller.account });
    await marketplace.write.acceptOffer([nexus.address, tokenId], { account: seller.account });

    // Verify results
    assert.equal(await nexus.read.ownerOf([tokenId]), getAddress(buyer.account.address));
    assert.equal(await nexusToken.read.balanceOf([seller.account.address]), offerPrice);
  });

  it("Should handle ERC1155 offers and acceptance", async function () {
    const { marketplace, mock1155, nexusToken, seller, buyer, owner } = await deployAll();
  ...
  it("Should revert on ERC721 offer if amount != 1", async function () {
    ...
    await nexus.write.donate(["nft", ""], { account: seller.account, value: parseEther("0.1") });
    const tokenId = 0n;

    const offerPrice = parseEther("1");
    await nexusToken.write.approve([marketplace.address, offerPrice], { account: buyer.account });
    await marketplace.write.createOffer([nexus.address, tokenId, 2n, offerPrice], { account: buyer.account });

    await nexus.write.approve([marketplace.address, tokenId], { account: seller.account });
    await assert.rejects(
        marketplace.write.acceptOffer([nexus.address, tokenId], { account: seller.account }),
