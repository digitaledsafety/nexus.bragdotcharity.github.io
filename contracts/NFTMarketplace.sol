...
contract NFTMarketplace is ReentrancyGuard {
    ...
    // Mapping from NFT contract -> Token ID -> Buyer Address -> Offer
    ...
    event OfferRejected(address indexed nftContract, uint256 indexed tokenId, address indexed seller, address buyer, uint256 price);
    ...
    function acceptOffer(address nftContract, uint256 tokenId, address buyer) external nonReentrant {
        ...
        emit OfferAccepted(nftContract, tokenId, msg.sender, offer.price, offer.amount, buyer);
    }

    /**
     * @notice Reject an offer and refund the buyer immediately
     * @param nftContract Address of the NFT contract
    ...
    function rejectOffer(address nftContract, uint256 tokenId, address buyer) external nonReentrant {
        ...
        emit OfferRejected(nftContract, tokenId, msg.sender, buyer, offer.price);
    }

    /**
     * @notice Cancel an offer you made (only after refund period)
     * @param nftContract Address of the NFT contract
     * @param tokenId ID of the token for which the offer was made
     * @param buyer Address of the buyer (must be msg.sender)
     */
    function cancelOffer(address nftContract, uint256 tokenId, address buyer) external nonReentrant {
        require(buyer == msg.sender, "Can only cancel your own offer");
        Offer memory offer = offers[nftContract][tokenId][buyer];
