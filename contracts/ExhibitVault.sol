...
contract ExhibitVault is ERC721Holder, ERC1155Holder, ReentrancyGuard {
    ...
    function onERC1155BatchReceived(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            uint256 id = ids[i];
            balances1155[nftContract][id][actualOwner] += values[i];
            // Only increase expiry
            if (newExpiry > expiry1155[nftContract][id][actualOwner]) {
                expiry1155[nftContract][id][actualOwner] = newExpiry;
            }
            emit Exhibited1155(nftContract, id, actualOwner, values[i], location, expiry1155[nftContract][id][actualOwner]);
        }
